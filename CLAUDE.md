...
# Loaner - Loan Tracking iOS App

**Purpose:** Track investments/loans where you are the lender, calculating daily compound interest and managing payments allocated between principal and interest.

**Current Status (Nov 13, 2025):**
- ✅ MVP Implementation Complete - All code features done
- ✅ Builds successfully with zero errors
- ✅ Onboarding flow implemented and working
- 🔄 Device testing in progress
- ⏳ Awaiting App Store assets (icon, screenshots)
- 📋 See `SESSION_SUMMARY.md` for detailed status
- 📋 See `MVP_SHIPPING_PLAN.md` for shipping checklist

**Key Features:**
- Multi-loan portfolio tracking
- Daily compound interest calculation (simplified to daily-only in v1.0)
- Payment entry with interest-first, principal-only, or custom split allocation
- Real-time payment impact preview
- CSV export for tax records
- Sample loan feature to explore the app
- First-launch onboarding experience
- Contextual help buttons
- Delete confirmation for safety
- Comprehensive input validation

**Simplified Data Model (v1.0):**
- `Loan` - Borrower name, start date, interest rate (always daily compounding), and transactions
  - **Removed in v1.0:** `compoundingFrequency` (now always daily), `status` (too complex for MVP)
- `Transaction` - Either capital addition (increases principal) or payment (reduces interest/principal)
- `LoanCalculator` - Pure function library for daily interest calculations and payment preview
  - **Simplified in v1.0:** All functions only support daily compounding

**Recent Changes (Nov 13, 2025):**
- Removed compounding frequency picker (now always daily)
- Removed loan status tracking (simplified UX)
- Added delete confirmation dialog
- Enhanced date validation (no future dates, no dates before loan start)
- Interest rate bounds checking (0.1% - 50%)
- Created `HelpButton.swift` reusable component
- Improved empty state with "Try Sample Loan" button
- Added sample loan creation feature
- Implemented onboarding flow with first-launch detection
- **Bug Fix:** Fixed "Get Started" button not dismissing onboarding

**Important Files:**
- `LoanerPackage/Sources/LoanerFeature/Models/` - Data models and calculation engine
- `LoanerPackage/Sources/LoanerFeature/Views/` - All SwiftUI views
  - `OnboardingView.swift` - First-launch experience (NEW)
  - `HelpButton.swift` - Contextual help component (NEW)
- `SESSION_SUMMARY.md` - Latest development session notes
- `MVP_SHIPPING_PLAN.md` - Step-by-step shipping checklist
- `APP_STORE_COPY.md` - Complete App Store listing content
- `COMPREHENSIVE_REVIEW.md` - Full app analysis and recommendations

**Known Issues:**
- None currently - all bugs fixed as of Nov 13, 2025

**Next Steps for Shipping:**
1. Continue device testing (in progress)
2. Create app icon (1024×1024)
3. Take 5 screenshots for App Store
4. Publish privacy policy to website
...
- **Entitlements File**: `Config/Loaner.entitlements` contains all app capabilities
...
To add capabilities to your app, edit `Config/Loaner.entitlements`:
